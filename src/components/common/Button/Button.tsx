...
const sizeStyle = {
  ...
  large: 'rounded-2xl py-11 px-16 text-4xl',
  none: '',
};
...
export function Button({ className = '', size = 'none', ...props }: Prop) {
  return (
    <button
      className={`flex justify-center items-center text-main border border-main hover:text-white hover:bg-main ${className} ${sizeStyle[size]}`}
      {...props}
    />
  );
}
