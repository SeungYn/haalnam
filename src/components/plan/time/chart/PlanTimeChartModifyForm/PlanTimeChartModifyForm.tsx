...
type Props = {
	closePopUp: () => void;
	originTimes: Plan[];
	mutatePlan: ReturnType<typeof usePatchUptatePlan>;
	currentPlanPageId: number;
	selectedPlanId: number;
	originSubject: string;
	startAngles: [number, number];
	endAngles: [number, number];
};
...
export default function PlanTimeChartModifyForm({
	closePopUp,
	originTimes,
	currentPlanPageId,
	mutatePlan,
	...
	const onSetCurrentAngle = (x: number, y: number) => {
		...
		let overlappingTime = checkStartOverlappingTime(originTimes, angleData[1]);
		...
		if (!angles.start) {
			...
			overlappingTime = checkEndOverlappingTime(originTimes, angleData[1]);
			if (overlappingTime) {
				// 라디안에서 1분 빼줌
	...
	const onSubmit = (e: FormEvent<HTMLFormElement>) => {
		...
		mutatePlan.mutate({
			...
			startTimeAngles: angles.start,
			endTimeAngles: angles.end,
		});
	};
	...
	return (
		<div className="flex flex-col items-center">
			...
			<div className="relative flex items-center justify-center py-14">
				<div className="absolute left-1/2 top-1/2 z-40 -translate-x-1/2 -translate-y-1/2 opacity-60">
					<PlanTimeChart times={originTimes} mode="Add" />
				</div>
				...
				{/* 눈금 별 시간 표시 */}
				{makeChartGradutionTimeInfo(
					(addChartWidth + addChartWidth * 0.12) / 2
				).map((v) => {
					return (
						<div
			...
			<form onSubmit={onSubmit}>
				...
				<div className="relative mb-20">
					<Button
						size="medium"
						className="w-full"
						isLoading={mutatePlan.isLoading}
